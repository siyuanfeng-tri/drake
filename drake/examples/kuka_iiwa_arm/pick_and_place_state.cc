...
EnvState::EnvState(const std::string& iiwa_model_path, const std::string end_effector_name, lcm::LCM* lcm)
    : iiwa_model_path_(iiwa_model_path), ee_name_(end_effector_name), lcm_(lcm) {
  iiwa_time_ = -1;
...
void EnvState::HandleIiwaStatus(const lcm::ReceiveBuffer* rbuf,
  ...
  if (iiwa_time_ == -1) {
    auto base_frame = std::allocate_shared<RigidBodyFrame<double>>(
        Eigen::aligned_allocator<RigidBodyFrame<double>>(), "world", nullptr, iiwa_base_);
  ...
  iiwa_ee_vel_ = iiwa_->CalcBodySpatialVelocityInWorldFrame(cache, *end_effector_);
}
...
void IiwaMove::MoveJoints(const EnvState& est_state, const std::vector<double>& time, const MatrixX<double>& q) {
  std::vector<int> info(time.size(), 1);
  robotlocomotion::robot_plan_t plan = EncodeKeyFrames(iiwa_, time, info, q);
