...
class EnvState {
 public:
   ...
   * place scenario.
   */
  EnvState(const std::string& iiwa_model_path, const std::string end_effector_name, lcm::LCM* lcm);
  ...
  const Isometry3<double>& get_iiwa_end_effector_pose() const { return iiwa_ee_pose_; }
  const Vector6<double>& get_iiwa_end_effector_velocity() const { return iiwa_ee_vel_; }
 ...
 private:
  ...
  void HandleWsgStatus(const lcm::ReceiveBuffer* rbuf,
      const std::string& chan,
...
class Action {
 public:
  explicit Action(lcm::LCM* lcm) : lcm_(lcm) {
    Reset();
  }
...
class IiwaMove : public Action {
 public:
   ...
   */
  IiwaMove(const RigidBodyTree<double>& iiwa, const std::string& channel, lcm::LCM* lcm)
      : Action(lcm),
        iiwa_(iiwa),
        pub_channel_(channel) {}
   ...
   */
  void MoveJoints(const EnvState& est_state, const std::vector<double>& time, const MatrixX<double>& q);
