...
int QPController::Control(const HumanoidStatus& rs, const QPInput& input,
  ...
  cost_comdd_->UpdateConstraint(
      ...
      input.w_com * rs.J_com().transpose() * (rs.Jdot_times_v_com() - input.comdd_d()));

  // For pelvisdd
  cost_pelvdd_->UpdateConstraint(
      input.w_pelv * rs.pelv().J().transpose() * rs.pelv().J(),
      input.w_pelv * rs.pelv().J().transpose() * (rs.pelv().Jdot_times_v() - input.pelvdd_d()));

  // For torsodd
  cost_torsodd_->UpdateConstraint(
      input.w_torso * rs.torso().J().transpose() * rs.torso().J(),
      input.w_torso *
          rs.torso().J().transpose() * (rs.torso().Jdot_times_v() - input.torsodd_d()));
  ...
  for (int i = 0; i < 2; i++) {
    cost_footdd_[i]->UpdateConstraint(
        ...
        input.w_foot *
            rs.foot(i).J().transpose() * (rs.foot(i).Jdot_times_v() - input.footdd_d(i)));
  }
  ...
  output->costs().resize(costs.size());
  int ctr = 0;
  for (auto cost_b : costs) {
    ...
    output->costs(ctr).first = cost->get_description();
    output->costs(ctr).second = val(0);
    ctr++;
  }
