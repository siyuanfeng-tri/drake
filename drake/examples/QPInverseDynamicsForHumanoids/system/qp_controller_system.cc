...
QPControllerSystem::QPControllerSystem(const RigidBodyTree<double>& robot) : robot_(robot) {
...
void QPControllerSystem::DoCalcOutput(const systems::Context<double>& context,
...
std::unique_ptr<systems::SystemOutput<double>> QPControllerSystem::AllocateOutput(
    const systems::Context<double>& context) const {
  std::unique_ptr<systems::LeafSystemOutput<double>> output(
      new systems::LeafSystemOutput<double>);
  QPOutput out(robot_);
  output->add_port(std::unique_ptr<systems::AbstractValue>(new systems::Value<QPOutput>(out)));
