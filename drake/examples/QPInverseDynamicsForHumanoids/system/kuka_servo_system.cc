...
KukaServoSystem::KukaServoSystem(const RigidBodyTree<double>& robot,
  ...
  // Debug is (q, v, vd)_nomianl, vd_feedback.
  output_port_index_debug_info_ =
      DeclareOutputPort(systems::kVectorValued,
                        get_robot().get_num_positions() +
                            3 * get_robot().get_num_velocities())
          .get_index();
...
void KukaServoSystem::DoCalcOutput(
  ...
  // Populated debugging info: (q, v, vd)_nominal + vd_feedback
  const VectorSetpoint<double>& plan = context.get_abstract_state<VectorSetpoint<double>>(get_abstract_state_index_plan());

  int dim = get_robot().get_num_positions();
  auto out_vector = GetMutableOutputVector(output, output_port_index_debug_info_);
  // nominal q
  out_vector.segment(0, dim) = plan.desired_position();
  // nominal v
  out_vector.segment(dim, dim) = plan.desired_velocity();
  // nominal vd
  out_vector.segment(2 * dim, dim) = plan.desired_acceleration();
  // qpinput' vd

  const QpInput& qp_input = context.get_abstract_state<QpInput>(get_abstract_state_index_qp_input());
  out_vector.segment(3 * dim, dim) = qp_input.desired_dof_motions().values();
}
