...
class KukaInverseDynamicsServo : public systems::Diagram<double> {
 public:
  KukaInverseDynamicsServo(
    ...
    // generate qp_input from nominal q and qd
    ...
    builder.ExportInput(rs_wrapper_->get_input_port_state());

    // expose nominal q qd input.
  ...
  inline const systems::InputPortDescriptor<double>& get_input_port_iiwa_state()
      const {
    return get_input_port(0);
  }

  inline const systems::InputPortDescriptor<double>& get_input_port_nominal_iiwa_state()
      const {
