...
void HumanoidManipulationPlan<T>::HandlePlanMessageGenericPlanDerived(
  ...
  VectorX<T> u(robot.get_num_actuators());
  Vector6<T> l_foot_wrench, r_foot_wrench;
  ...
  for (size_t f = 0; f < msg.plan.size(); ++f) {
    ...
    // TODO only doing RPY joints now.
    DecodeRobotStateLcmMsg(keyframe, robot_status.name_to_position_index(),
                           &time, &q, &v, &u, &l_foot_wrench, &r_foot_wrench);
  ...
  // Generates dof trajectories.
  {
    ...
    PiecewiseCubicTrajectory<T> dof_traj(
  ...
  {
    const Vector3<T> zero3 = Vector3<T>::Zero();
