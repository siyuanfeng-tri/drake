...
void RobotStateLcmMessageTranslator::ResizeMessage(
  ...
  if (msg->joint_name.size() != num_1dof_q) msg->joint_name.resize(num_1dof_q);
  if (msg->joint_position.size() != num_1dof_q)
    msg->joint_position.resize(num_1dof_q);
  if (msg->joint_velocity.size() != num_1dof_q)
    msg->joint_velocity.resize(num_1dof_q);
  if (msg->joint_effort.size() != num_1dof_q)
