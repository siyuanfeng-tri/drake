...
void Simulator<T>::StepTo(const T& boundary_time) {
  ...
  auto external_events = system_.AllocateCompositeEventCollection();
  ...
  while (context_->get_time() < boundary_time || sample_time_hit) {
    ...
    // Now checks all the events generated by some src_system for some dest_system.
    {
      // This is a flat version of the collection of `dest_system + its events` described above.
      std::unordered_map<const System<T>*, std::unique_ptr<LeafCompositeEventCollection<T>>> flat_external_events;
      // T is when flat_external_events happens.
      ...
      if (next_sample_time < next_ext_event_time) {
        ...
        // Need to convert the flat flat_external_events to the same tree structure, if flat_external_events contains
        // some random ass system that system_ doesn't know about, those events will be
        // ignored. Then do a merge.
        system_.ConvertLeafCompositeEventCollection(flat_external_events, external_events.get());
        timed_events->Merge(*external_events);
      } else {
        system_.ConvertLeafCompositeEventCollection(flat_external_events, external_events.get());
        timed_events->SetFrom(*external_events);
